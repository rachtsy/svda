...
accelerate launch --config_file=accelerate_configs/deepspeed_zero2.yaml \
  ...
  --output_dir="logs/fine-tuning-attack/pure_good/ablation-4H-redoREF-Llama2-7b-chat-k-${k_dim}-q-${q_factor}-fix-${fixed_PHD}-${proj_init}" \
  --logging_steps=1 \
  --num_train_epochs=25 \
  ...
  --proj_layer $layer \
  --proj_factor $fact \
  --job_name ablation-4H-redoREF-Llama2-7b-chat-k-${k_dim}-q-${q_factor}-fix-${fixed_PHD}-head-${head}-${proj_init}
...
python3 -m pipeline.run_pipeline --model_path /root/rachel/shallow-vs-deep-alignment/logs/fine-tuning-attack/pure_good/ablation-4H-redoREF-Llama2-7b-chat-k-${k_dim}-q-${q_factor}-fix-${fixed_PHD}-${proj_init} \
    --k_dim $k_dim --q_factor $q_factor --fixed_PHD $fixed_PHD --head $head --proj_init gen${proj_init} --proj_layers ALL --proj_num_heads 4 \
