...
accelerate launch --config_file=accelerate_configs/deepspeed_zero2.yaml \
  ...
  --output_dir="logs/fine-tuning-attack/pure_good/PG-gemma-7b-it-k-${k_dim}-q-${q_factor}-fix-${fixed_PHD}-head-${head}-${proj_init}" \
  --logging_steps=1 \
  --num_train_epochs=$i \
...
python3 -m pipeline.run_pipeline --model_path /root/rach/shallow-vs-deep-alignment/logs/fine-tuning-attack/pure_good/PG-gemma-7b-it-k-${k_dim}-q-${q_factor}-fix-${fixed_PHD}-head-${head}-${proj_init} \
